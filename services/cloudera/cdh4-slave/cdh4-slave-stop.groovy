...
if(context.attributes.thisApplication.useMongo == true) {
	...
	println "cdh4-node-stop.groovy: shuting down the mongod instance"
	...
	println "cdh4-node-stop.groovy: result is ${result}"
	
	if(!result.ok) {

		datadir = context.attributes.thisInstance.mongoDataDir
		logDir = context.attributes.thisInstance.mongoLogDir
		pid = new File("${datadir}/mongod.lock").readLines()[0].toInteger()

		builder.sequential {
			exec(executable:"sudo", output:"${logDir}/mongodb_stop.log", osfamily:"unix"){
				arg value: "kill" 
				arg line:"-9"
				arg line:"${pid}"
			}
